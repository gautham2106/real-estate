...
const negotiatingOrSold = mockProperties.filter(
  (p) => p.status === 'Negotiating' || p.status === 'Sold'
).length
...
const columns = [
  ...
  { key: 'type', header: 'Type', sortable: true },
  {
    key: 'area',
    header: 'Area',
    render: (row: Property) => `${row.area} ${row.area_unit}`,
  },
  {
    key: 'price',
    header: 'Price',
  ...
  },
  {
    key: 'status',
    header: 'Status',
    render: (row: Property) => <Badge status={row.status} />,
  },
  {
    key: 'district',
    header: 'District',
    render: (row: Property) => row.district ?? '—',
  },
  {
    key: 'assigned_broker_id',
    header: 'Assigned Broker',
    render: (row: Property) =>
      row.assigned_broker_id ? brokerMap[row.assigned_broker_id] ?? row.assigned_broker_id : '—',
  },
  {
    key: 'action',
    header: 'Action',
  ...
  return (
    <div className="space-y-6 max-w-screen-xl">
      <PageHeader
        ...
        subtitle={`${totalProperties} properties in the system`}
        action={
          <Link
            href="/properties/new"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        />
        <StatCard
          title="Negotiating / Sold"
          value={negotiatingOrSold}
      ...
      <DataTable<Record<string, unknown>>
        data={mockProperties as unknown as Record<string, unknown>[]}
