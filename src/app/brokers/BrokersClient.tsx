...
export default function BrokersClient({ brokers }: BrokersClientProps) {
  ...
  const totalDealsClosed = brokers.reduce((s, b) => s + (b.deals_closed ?? 0), 0)
  const totalCommissionPaid = brokers.reduce((s, b) => s + (b.total_commission_earned ?? 0), 0)
          ...
          >
            ...
            {status !== 'All' && (
              <span className="ml-1 opacity-70">
                ({brokers.filter((b) => b.status === status).length})
              </span>
            )}
          </button>
        ))}
