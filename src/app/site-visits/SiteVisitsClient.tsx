...
export default function SiteVisitsClient({ siteVisits, properties, buyerLeads, brokers }: SiteVisitsClientProps) {
  const [filter, setFilter] = useState<ReactionFilter>('All')

  const enriched = siteVisits.map(v => ({
    ...v,
    property: properties.find(p => p.id === v.property_id),
    buyer: buyerLeads.find(b => b.id === v.buyer_id),
    broker: brokers.find(b => b.id === v.broker_id),
  }))
  ...
  return (
    <div className="space-y-6 max-w-screen-xl">
      <PageHeader
        title="Site Visits"
        subtitle={`${siteVisits.length} recorded visits`}
        action={
          <Link
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Visits', value: siteVisits.length, color: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'Interested', value: siteVisits.filter(v => v.buyer_reaction === 'Interested').length, color: 'text-green-600', bg: 'bg-green-50' },
          { label: 'Negotiating', value: siteVisits.filter(v => v.buyer_reaction === 'Negotiating').length, color: 'text-yellow-700', bg: 'bg-yellow-50' },
          { label: 'Progressed', value: siteVisits.filter(v => v.outcome === 'Progressed').length, color: 'text-purple-600', bg: 'bg-purple-50' },
        ].map(s => (
